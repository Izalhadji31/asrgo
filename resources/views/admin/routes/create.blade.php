...
@section('content')
    <div class="space-y-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h1 class="font-[Barlow_Condensed] text-3xl font-semibold text-blue-900">Tambah Rute</h1>
            <p class="mt-1 text-sm text-slate-500">Buat rute baru untuk layanan Travel. Tentukan mitra berdasarkan kota keberangkatan setiap sesi.</p>
        </div>
            ...
            <form action="{{ route('admin.routes.store') }}" method="POST">
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="origin" value="{{ old('origin') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 transition focus:border-[#E8A33D] focus:outline-none focus:ring-2 focus:ring-[#E8A33D]/20" placeholder="Contoh: Jakarta" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="destination" value="{{ old('destination') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 transition focus:border-[#E8A33D] focus:outline-none focus:ring-2 focus:ring-[#E8A33D]/20" placeholder="Contoh: Bandung" required>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price" value="{{ old('price') }}" class="w-full rounded-xl border border-slate-300 py-2.5 pl-10 pr-4 text-sm text-slate-800 transition focus:border-[#E8A33D] focus:outline-none focus:ring-2 focus:ring-[#E8A33D]/20" placeholder="0" required>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="mitra_id_pagi" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 transition focus:border-[#E8A33D] focus:outline-none focus:ring-2 focus:ring-[#E8A33D]/20">
                                ...
                                @foreach ($mitras as $mitra)
                                    <option value="{{ $mitra->id }}" @selected(old('mitra_id_pagi') == $mitra->id)>{{ $mitra->name }} — {{ $mitra->vehicles_count }} kendaraan</option>
                                @endforeach
                            </select>
                    ...
                    <div class="md:col-span-2 rounded-xl border border-orange-200 bg-orange-50/50 p-5">
                        ...
                        <div>
                            ...
                            <select name="mitra_id_siang" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 transition focus:border-[#E8A33D] focus:outline-none focus:ring-2 focus:ring-[#E8A33D]/20">
                                <option value="">— Pilih mitra (opsional) —</option>
                                @foreach ($mitras as $mitra)
                                    <option value="{{ $mitra->id }}" @selected(old('mitra_id_siang') == $mitra->id)>{{ $mitra->name }} — {{ $mitra->vehicles_count }} kendaraan</option>
                                @endforeach
                            </select>
                ...
                <div class="mt-8 flex items-center gap-3 border-t border-slate-100 pt-6">
                    <button type="submit" class="rounded-xl bg-[#3F7D6C] px-6 py-2.5 text-sm font-medium text-white transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-[#3F7D6C] focus:ring-offset-2">Simpan Rute</button>
