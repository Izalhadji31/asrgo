        ...
        <aside class="w-full bg-blue-900 text-slate-100 lg:w-72">
            <div class="flex items-center justify-between border-b border-white/10 px-6 py-6">
                <div>
                    ...
                    <p class="text-sm text-slate-400">Rental Armada</p>
                </div>
                <div class="rounded-full bg-[#E8A33D] px-3 py-1 text-xs font-semibold text-blue-900">Admin</div>
            </div>

            <nav class="space-y-2 px-4 py-6" aria-label="Sidebar Navigation">
                @php
                $menu = [
                    ['label' => 'Ringkasan', 'icon' => 'fa-solid fa-gauge-high', 'route' => 'admin.dashboard', 'active' => ['admin.dashboard']],
                    ['label' => 'Papan Booking', 'icon' => 'fa-solid fa-clipboard-list', 'route' => 'admin.bookings.index', 'active' => ['admin.bookings.index']],
                    ...
                    ['label' => 'Riwayat Aktivitas', 'icon' => 'fa-solid fa-clock-rotate-left', 'route' => 'admin.audit-logs.index', 'active' => ['admin.audit-logs.index']],
                ...
                <a href="{{ route($item['route']) }}"
                    class="group flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-[#E8A33D] focus:ring-offset-2 focus:ring-offset-blue-900 {{ $isActive ? 'bg-white/10 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">
                    <span class="text-base text-[#E8A33D]"><i class="{{ $item['icon'] }}"></i></span>
        ...
        <div class="flex-1">
            <header class="relative border-b border-slate-200 bg-white/80 backdrop-blur">
                <div class="flex flex-col gap-3 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
                    <div>
                        <p class="font-[Barlow_Condensed] text-2xl font-semibold text-blue-900">Dashboard Admin</p>
                        <p class="text-sm text-slate-500">Pantau booking, mitra, armada, dan pendapatan secara real-time.</p>
                    </div>
                    <div class="flex items-center gap-3" x-data="{ open: false }" @click.outside="open = false">
                        <button @click="open = !open" class="flex items-center gap-2 rounded-full border border-slate-200 bg-[#F5F4F0] px-3 py-2 text-sm text-slate-600 transition hover:border-[#E8A33D] hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-[#E8A33D]">
