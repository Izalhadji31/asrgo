...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - Mitra</title>
...
<body class="min-h-screen bg-[#F5F4F0] text-slate-800 antialiased">
    <div class="min-h-screen lg:flex">
        <aside class="w-full bg-[#0F172A] text-slate-100 lg:w-72">
            <div class="flex items-center justify-between border-b border-white/10 px-6 py-6">
                <div>
                    ...
                    <p class="text-sm text-slate-400">Panel Mitra</p>
                </div>
                <div class="rounded-full bg-[#3F7D6C] px-3 py-1 text-xs font-semibold text-white">Mitra</div>
            </div>
            ...
            <nav class="space-y-2 px-4 py-6" aria-label="Sidebar Navigation">
                ...
                <a href="{{ route($item['route']) }}"
                    class="group flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-[#3F7D6C] focus:ring-offset-2 focus:ring-offset-[#0F172A] {{ $isActive ? 'bg-white/10 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">
                    <span class="text-base text-[#3F7D6C]"><i class="{{ $item['icon'] }}"></i></span>
        ...
        <div class="flex-1">
            <header class="relative border-b border-slate-200 bg-white/80 backdrop-blur">
                <div class="flex flex-col gap-3 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
                    <div>
                        <p class="font-[Barlow_Condensed] text-2xl font-semibold text-slate-900">Panel Mitra</p>
                        <p class="text-sm text-slate-500">Kelola armada dan pantau pendapatan Anda.</p>
                    </div>
                    <div class="flex items-center gap-3" x-data="{ open: false }" @click.outside="open = false">
                        <button @click="open = !open" class="flex items-center gap-2 rounded-full border border-slate-200 bg-[#F5F4F0] px-3 py-2 text-sm text-slate-600 transition hover:border-[#3F7D6C] hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-[#3F7D6C]">
                            <span>{{ auth()->user()?->name ?? 'Mitra' }}</span>
